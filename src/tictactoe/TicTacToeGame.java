...
/**
 * The model of tic tac toe game maintains state of the game, evaluates moves,
...
public class TicTacToeGame {
	...
	public Player winner() {
		...
		for (int n = 0; n < boardsize; n++) {
			p = pieces[0][0].type;
			if (p != Player.NONE && p == pieces[n][n].type) {
				count++;
			}
			if (count == boardsize) {
				return p;
		...
		int save = boardsize - 1;
		for (int n = 0; n < boardsize; n++) {
			p = pieces[0][save].type; // start at lower-left corner
			if (p != Player.NONE && p == pieces[n][save - n].type) {
				count++;
			}
			if (count == boardsize) {
				return p;
